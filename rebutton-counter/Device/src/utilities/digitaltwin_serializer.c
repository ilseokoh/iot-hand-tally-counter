...
bool Counter_SerializeHitCountTelemetry(char * payloadBuffer, int size)
{
    ...
    COUNTER_HITCOUNT hitCount;
    Counter_Telemetry_ReadHitCount(&hitCount);

    JSON_Value *root_value = json_value_init_object();
    JSON_Object *root_object = json_value_get_object(root_value);

    json_object_set_number(root_object, "hit", hitCount.hit);

    json_object_set_string(root_object, "hitTime", hitCount.hitTime);
    ...
    {
        LogError("Failed to build hitCount payload string");
        return false;
    }
    else
    {
...
bool Counter_SerializeTypeProperty(char * payloadBuffer, int size)
{
    ...
    char* type = Counter_Property_GetType();

    int neededSize = snprintf(NULL, 0, "\"%s\"", type);

    if (neededSize > size - 1)
    {
        LogError("Failed to build Type payload string");
        return false;
    }
    else
    {
        snprintf(payloadBuffer, size, "\"%s\"", type);
    }
...
bool Counter_SerializeResetTimeProperty(char * payloadBuffer, int size)
{
    ...
    char* resetTime = Counter_Property_GetResetTime();

    int neededSize = snprintf(NULL, 0, "\"%s\"", resetTime);

    if (neededSize > size - 1)
    {
        LogError("Failed to build ResetTime payload string");
        return false;
    }
    else
    {
        snprintf(payloadBuffer, size, "\"%s\"", resetTime);
    }
...
bool Counter_SerializeTotalCountProperty(char * payloadBuffer, int size)
{
    ...
    int totalCount = Counter_Property_GetTotalCount();

    int neededSize = snprintf(NULL, 0, "%d", totalCount);

    if (neededSize > size - 1)
    {
        LogError("Failed to build TotalCount payload string");
        return false;
    }
    else
    {
        snprintf(payloadBuffer, size, "%d", totalCount);
    }
...
bool Counter_SerializeResetCountCommandResponse(COUNTER_RESETCOUNT_ResetResponse* response, char* payloadBuffer, int size)
{
    ...
    json_object_set_string(root_object, "LastCountTime", response->LastCountTime);

    json_object_set_number(root_object, "LastCount", response->LastCount);
    ...
    if (serializedString == NULL || neededSize > size - 1)
    {
        LogError("Failed to build ResetCount payload string");
        return false;
    }
    else
    {
